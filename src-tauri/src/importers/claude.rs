...
pub fn sync(conn: &Connection, incremental: bool) -> ImporterStats {
    ...
    for entry in WalkDir::new(&claude_dir).into_iter().filter_map(|e| e.ok()) {
        ...
        let cid = format!("claude:{}", path.file_stem().unwrap_or_default().to_string_lossy());
...
fn parse_claude_jsonl(cid: &str, path: &Path) -> Result<Option<RawConversation>, Box<dyn std::error::Error>> {
    ...
    for line in reader.lines() {
        ...
        match msg_type {
            "user" | "prompt" => {
                ...
                if let Some(s) = val.get("text").or_else(|| val.get("content")).or_else(|| val.get("prompt")).and_then(|v| v.as_str()) {
            ...
            "assistant" | "completion" => {
                ...
                if let Some(s) = val.get("text").or_else(|| val.get("content")).and_then(|v| v.as_str()) {
                ...
                let thinking_str = if thinking_parts.is_empty() { None } else { Some(thinking_parts.join("\n\n")) };
                let tool_args = if tool_calls.is_empty() { None } else { serde_json::to_string_pretty(&tool_calls).ok() };
                let tool_name = if tool_calls.len() == 1 {
                    tool_calls[0].get("name").and_then(|v| v.as_str()).map(|s| s.to_string())
                ...
                messages.push(RawMessage {
                    ...
                    message_type: if tool_args.is_some() { "tool_call".to_string() } else { "text".to_string() },
                    content: if content_str.is_empty() { tool_args.clone().unwrap_or_default() } else { content_str },
