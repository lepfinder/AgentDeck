...
### 变更
- **大盘甘特图改为标准时间条**：按会话当天消息的起止时间绘制矩形条并对齐 24h 轴；间隔超过 20 分钟自动拆条，不再把整段会话画成一根超长块
- **时长展示**：持续时间超过 60 分钟时显示为「x 小时 y 分钟」
- **Git 文件状态**：未跟踪文件状态符由 `?` 改为绿色 `U`；未跟踪目录递归列出内部文件，修复树节点文件名为空
- **Git diff 修正**：未暂存对比改为「暂存区 vs 工作区」，修改文件不再被整文件标成新增

## [0.3.12] - 2026-09-20

### 新增
- **额度悬浮条**：coding agent 用量改为独立置顶小窗，每个额度一档品牌色进度环，悬浮展开明细卡；顶部 logo 可拖到任意屏幕位置（含菜单栏区）并记忆位置，拔屏自动回退主屏
- **悬浮条显隐控制**：logo 右键可隐藏或退出，托盘与主窗口「余量」弹层提供恢复入口，托盘菜单文案随悬浮条当前状态自动切换
- **变更文件右键加入 .gitignore**：Git 看板文件树右键文件或目录，一键把忽略规则写入仓库根 `.gitignore`（目录自动带尾斜杠、去重、已跟踪路径给出提示）
- **批量总结重命名入口升级**：从会话列表文字链接改为项目 Header 右侧按钮（位于「Git 看板」左侧）

### 变更
- **顶栏精简**：移除产品名后的「Desktop」徽章，左上角只保留图标与 AgentDeck
- **菜单栏托盘瘦身**：额度读数不再占用系统菜单栏文字，托盘只保留显示主窗口、悬浮条显隐与退出操作

### 修复
- **新增文件 diff 预览为空**：未跟踪文件在详情面板显示「没有文本变更」，现改为全文按新增渲染（libgit2 pathspec 与 recurse_untracked_dirs 组合问题）
- **Git 看板中切换项目丢失视图**：点击左侧项目不再退回会话列表，保持 Git 看板并加载新项目变更树；左侧项目选中态在 Git 看板下正常高亮
- **侧栏项目卡悬浮重叠**：悬浮时 Git 待提交徽章与会话时间一起左移，不再互相压盖

## [0.3.7] - 2026-09-15

### 新增
- **工作区批量总结命名**：会话列表可一键批量生成短标题与摘要，支持仅未总结/失败重试/强制重做、进度面板与取消

### 修复
- **批量总结一点开始就全跳过**：修正「开始运行」误触发取消逻辑的问题
- **AgentDeck 退出导致子服务 EPIPE**：stdout/stderr 直接落到日志文件（续 0.3.6）
