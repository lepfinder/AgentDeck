...
### 新增
- **Antigravity 精确 token 用量**：直接解析会话库 `conversations/*.db` 内的 protobuf 用量数据，落库输入/缓存读/输出/思考四桶 token 与模型名（`gemini-default` 路由标签按显示名找回具体模型），历史全量约 74.9 亿 tokens
- **用量统计新维度**：新增「按项目」「按会话」两张表，各取 token 总量前 20；项目名优先显示用户改过的别名，无工作区的会话归入「—」
- **用量时间范围「今天」**：与 7 天 / 30 天 / 全部并列，统计本地今天零点起的用量

### 变更
- **用量数字中文单位**：token 数量级显示为 74.9亿 / 5593.7万，不足一万保持千分位；英文界面对应 K/M/B
- **按天表格倒序**：最新日期排最前
- **统计页布局**：默认时间范围改为 7 天；「按天」表格移至最前

### 修复
- **存量会话用量永不落库**：旧版会话行内容指纹为空时，保存层按时间戳与条数判定「未变化」而永久早退，带用量的解析写不进库；现空指纹行遇到带用量的解析强制重写一次（Antigravity 98 个、Hermes 385 个会话补全用量）
- **强制重导被不完整运行消耗**：9 个 importer 的解析版本标记统一改为「本次同步零错误才写入」，有错误的运行不再永久吃掉强制重导
- **清理历史空壳会话**：移除 40 条零消息、无来源文件的老版本残留行

## [0.3.14] - 2026-09-20

### 修复
- **额度悬浮条**：透明窗口四角不再露出主题灰底；明细「已用」移到进度条右上角，重置时间与剩余刷新时长左右分栏；Antigravity 进度环只统计 Gemini（明细仍展示 Claude/GPT）
- **甘特图**：悬浮提示与详情弹窗展示该时段涉及的 Agent 名称
